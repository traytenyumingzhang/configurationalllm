...
class ConfigManager:
        ...
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    # Ensure new default settings are present if loading an old config
                    default_keys = self._create_default_config(save=False) # Get defaults without saving
                    updated = False
                    for key, value in default_keys.items():
                        if key not in config_data:
                            config_data[key] = value
                            updated = True
                        # Special handling for nested dicts like api_settings
                            ...
                            for sub_key, sub_value in value.items():
                                if sub_key not in config_data[key]:
                                     ...
                                     updated = True
                    if updated:
                        self._save_config(config_data) # Save if new keys were added
                    return config_data
            except Exception as e:
                print(f"Error loading config, creating default: {e}")
                return self._create_default_config()
        else:
            return self._create_default_config()
    
    def _create_default_config(self, save=True):
        ...
        default_config = {
            "api_settings": {
                ...
                "model": "claude-3-7-sonnet-20250219", # Note: previous was claude-3-7-sonnet-20250219, check if this is intended
                "temperature": 0.7,
            ...
            "message": {
                "user_message": "Please analyze this document (File: {filename}, MD5: {md5}, Iteration: {iteration}) and summarize the key findings, methodology, and conclusions. If there are any tables or charts, describe their contents."
            },
            "language": "en_US",  # Default language
            "num_iterations": 1, # Added default
            "rate_limit_delay": 5, # Added default
            "last_opened_dir": os.path.expanduser("~") # Added default
        }
        if save:
    ...
    # Generic setting getter and setter
    def get_setting(self, key, default=None):
        """Get a generic setting from the config."""
        return self.config.get(key, default)

    def save_setting(self, key, value):
        """Save a generic setting to the config."""
