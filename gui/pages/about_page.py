...
class AboutPage(QWidget):
    ...
    def __init__(self, config_manager, main_window=None): 
        ...
        # Version info
        app_version = "1.0.1" # Default version
        if self.main_window and hasattr(self.main_window, 'APP_VERSION'):
            app_version = self.main_window.APP_VERSION
        version_label = QLabel(f"Version λ - {app_version}")
        ...
        font_note = QLabel("Note: This application uses system default fonts to avoid potential copyright issues.")
        font_note.setStyleSheet("color: gray; font-style: italic;"); font_note.setWordWrap(True)
        self.layout.addWidget(font_note)
        
        licenses_group = QGroupBox("Third-Party Licenses")
        licenses_layout = QVBoxLayout(licenses_group)
        license_intro = QLabel("This software incorporates third-party components under their respective licenses:")
        license_intro.setWordWrap(True); licenses_layout.addWidget(license_intro)
        
        third_party_licenses = [
            "Python: Python Software Foundation License", "PySide6: LGPL-3.0",
            "anthropic: MIT License", "openai: MIT License", "PyPDF2: BSD License",
            "pandas: BSD-3 License", "requests: Apache License 2.0", "google-generativeai: Apache License 2.0"
        ]
        for license_info in third_party_licenses:
            label = QLabel(f"- {license_info}"); label.setWordWrap(True); licenses_layout.addWidget(label)
        
        compliance_note = QLabel("Users must comply with all terms of these licenses when using this software.")
        compliance_note.setWordWrap(True); compliance_note.setStyleSheet("font-weight: bold;")
    ...
    def enter_page(self):
        # If main_window reference is available, update version string dynamically
        if self.main_window and hasattr(self.main_window, 'APP_VERSION'):
            version_widget = self.findChild(QLabel, "version_label") # Assuming we name it
            if not version_widget: # Fallback: find by text if not named
                 for child in self.findChildren(QLabel):
                     if child.text().startswith("Version λ -"):
                         version_widget = child
                         break
            if version_widget:
                version_widget.setText(f"Version λ - {self.main_window.APP_VERSION}")
        pass
